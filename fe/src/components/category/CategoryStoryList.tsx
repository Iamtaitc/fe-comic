// src/components/category/CategoryStoryList.tsx
"use client";

import { useEffect, useRef, useCallback, useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { useAppDispatch } from "@/store/hooks";
import { loadMoreStories } from "@/store/slices/categorySlice";
...
export function CategoryStoryList({
  stories,
  loading,
  pagination,
  categorySlug,
  filters
}: CategoryStoryListProps) {
  ...
  const handleLoadMore = useCallback(async () => {
    if (pagination.hasNextPage && !isLoadingMore) {
      ...
      await dispatch(loadMoreStories({
        slug: categorySlug,
  ...
  }, [dispatch, categorySlug, pagination.hasNextPage, pagination.currentPage, isLoadingMore, filters]);
      ...
      <div className="container py-16">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mb-6">
            Hiện tại chưa có truyện nào trong thể loại này hoặc không khớp với bộ lọc của bạn.
          </p>
          ...
          {!pagination.hasNextPage && stories.length > 0 && (
            ...
            >
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-accent/50 rounded-full text-sm text-muted-foreground">
                <span>🎉</span>
                <span>Bạn đã xem hết tất cả truyện trong thể loại này!</span>
              </div>
            </motion.div>
          )}
        </div>
        ...
        {pagination.totalPages > 1 && (
          ...
          >
            <div className="text-sm text-muted-foreground">
              Hiển thị {stories.length} / {pagination.totalStories.toLocaleString()} truyện
